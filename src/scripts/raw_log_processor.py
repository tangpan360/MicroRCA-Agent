...
3. 时间处理：
    - 标准化时间格式（@timestamp列转换为datetime类型）
...
def process_single_file(file_path):
    ...
    try:
        ...
        # 将@timestamp转换为datetime类型，然后创建纳秒级时间戳的新列
...
def main(cpu_ratio=0.3):
    ...
    # 使用绝对路径构建数据路径
    base_path = os.path.join(project_root, 'data', 'raw', '2025-06-*', 'log-parquet', '*.parquet')
